<?php
    ...
    if(isset($_POST['upload'])){
        if($_FILES['game_image']['size'] != 0 && $_FILES['cover_image']['size'] != 0 && isImage($_FILES['game_image']) && isImage($_FILES['cover_image'])){
            $title = $_POST['title'];
            ...
            try{
                ...
                header("Location: /Games_Library/Add_Game/Add_Game.php?message=success");
            }
            ...
            http_response_code(500);
            header("Location: /Games_Library/Add_Game/Add_Game.php?message=badfileformat");
        }
    }
    
    function isImage($file){
        $fileinfo = finfo_open(FILEINFO_MIME_TYPE);
        if (!in_array(finfo_file($fileinfo, $file['tmp_name']), array('image/jpeg', 'image/png','image/gif'))) {
            return false;
        }
        return true;
    }
